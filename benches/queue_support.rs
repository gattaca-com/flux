// Shared measurement machinery for the raw and Spine queue benchmarks.
use std::{
    collections::HashSet,
    env,
...
pub fn transfer<const B: usize>(
    sender: impl Tx<B>,
    receiver: impl Rx<B>,
    settings: &Settings,
    seed: u32,
    mut check_phase: impl FnMut(usize),
) -> Option<Measurement> {
...
pub fn quantile(sorted: &[u64], percent: usize) -> u64 {
...
pub fn summarize(queue_name: &str, bytes: usize, results: Vec<Measurement>) {
    if results.is_empty() {
        return;
    }
    let n = results.len();
    ...
    println!(
        "summary,{queue_name},{bytes},{n},{mean:.6},{sd:.6},{},{}",
        quantile(&samples, 50),
        quantile(&samples, 95)
    );
}
