...
/// A consumer the producer has lapped resyncs onto the value its slot holds
/// now and reads forward from there, instead of losing the ring to
/// `recover_after_error`'s jump to the write head.
#[test]
